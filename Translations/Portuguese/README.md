# .htaccess Snippets
Uma coleção de snippets .htaccess, tudo em um só lugar.

**Disclaimer**: Ao deixar cair o ano snippet `em .htaccess` arquivo é na maioria das vezes suficiente, há certos casos que alterações serão necessárias. Use por sua conta e risco.

**IMPORTANTE**: Apache 2.4 introduz algumas alterações significativas, principalmente na configuração de controle de acesso. Para mais informações, consulte o [documento atualização] (https://httpd.apache.org/docs/2.4/upgrading.html), bem como [essa questão] (https://github.com/phanan/htaccess/issues/2).

## Credits: 
O que estamos fazendo aqui é trechos mais útil de todo a web (por exemplo, uma boa parte é de [Apache Configs] (https://github.com/h5bp/server-configs-apache)) em um só lugar. Enquanto estivemos em tentar tentativa onde o crédito devido, as coisas podem estar em falta. Se você acreditar em qualquer coisa aqui é o seu trabalho e os créditos devem ser dados, deixe-nos saber, ou apenas enviar um PR.
...
```
Isso funciona para qualquer domínio. [Source](https://stackoverflow.com/questions/4916222/htaccess-how-to-force-www-in-a-generic-way)

### Force non-www
It's [still](http://www.sitepoint.com/domain-www-or-no-www/) [open](https://devcenter.heroku.com/articles/apex-domains) [for](http://yes-www.org/) [debate](http://no-www.org/) para dominio com www ou sem www aqui vai:
``` apacheconf
RewriteEngine on
...
# Nota: Também é recomendável habilitar HTTP Strict Transport Security (HSTS)
...
### Apelida(Alias) um Diretório específico
``` apacheconf
RewriteEngine On
RewriteRule ^source-directory/(.*) target-directory/$1
```

### Apelida(Alias) caminhos para o script [REVIEW,REVISAR]
``` apacheconf
FallbackResource /index.fcgi
```
Este exemplo tem um arquivo `index.fcgi` em algum diretório, e todos os pedidos dentro desse diretório que não conseguem resolver um diretório/nome do arquivo será enviado para o script `index.fcgi`. 
É bom se você quer `baz.foo/some/cool/path` a ser feitos pelo `baz.foo/index.fcgi` (que também suporta pedidos de `baz.foo`), mantendo` baz.foo/css/style.css` e semelhantes. 
Tenha acesso ao caminho original da variável de ambiente PATH_INFO, como exposto no seu ambiente de script.
...
```
Esta é uma versão menos eficiente da directiva FallbackResource (porque o uso do `mod_rewrite` é mais complexa do que apenas lidar com a directiva `FallbackResource`), mas também é mais flexível.
